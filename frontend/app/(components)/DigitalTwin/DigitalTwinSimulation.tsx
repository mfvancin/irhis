...
export const DigitalTwinSimulation: React.FC<DigitalTwinSimulationProps> = ({ patientId }) => {
  ...
  const handleRunSimulation = async () => {
    ...
    try {
      const simulationParams: SimulationParams = {
        ...
        duration,
        parameters,
        exerciseIntensity: parameters.exerciseIntensity || 0,
        recoveryTime: parameters.recoveryTime || 0
      };
  ...
  const pollSimulationStatus = async (simulationId: string) => {
    const interval = setInterval(async () => {
      try {
        const response = await digitalTwinService.getSimulationResult(simulationId);
